...
// console.log(copyChoice)
  ...
  const searchResult = () => {
    ...
    const FilterAll = data && data.filter((e, i) => {
      ...
      const etcRemove = !e.kindCd.includes("[기타]");
      ...
      if (isChoice[0] === 0 && isChoice[1] === 0 && isChoice[2] === 0) {
        ...
        // 모든 지역이 아니고 모든 축종 모든 품종 
      }else if(isChoice[0] === 1 && isChoice[1] === 0 && isChoice[2] === 0){
        return cityValueArray 
        
      }
      else if(isChoice[0] ===  1 && subValueArray  && isChoice[2]=== 1)
      {
        // 모든 지역이 아니고 축종이 바뀌고 품종이 바뀔때
        return  cityValueArray& subValueArray && KindValueArray
      }
      // 축종만 바뀔때 
      // else if(idsChoise[0] === 0 ){
      //   return
      // }
      // // else if(isChoice[0]===1 && subValueArray && isChoice[2]===0){
      //   return subValueArray
      // }
    
      // 모든 지역이고 축종이 바뀌고 품종이 바뀔때 
      // else if(isChoice[0] === 1 && isChoice[1] === 1 && isChoice[2]=== 1){
      //   return KindValueArray
      // }
